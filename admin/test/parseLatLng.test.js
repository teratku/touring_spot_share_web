...
// MARK: GoogleマップのURLをそのまま貼る（実機で要望）

test("場所を開いたURLをそのまま貼れる", () => {
  // ⚠️ 実際に使われた URL そのもの。度分秒の表記・%エンコード・ズーム値・
  //    末尾のパラメータが全部入っていても、狙った座標が取れること
  const url = "https://www.google.com/maps/place/35%C2%B037'17.4%22N+139%C2%B050'06.4%22E"
    + "/@35.621501,139.835122,16z/data=!4m4!3m3!8m2!3d35.6215013!4d139.8351224"
    + "?hl=ja-JP&entry=ttu&g_ep=EgoyMDI2MDgxMi4wIKXMDSoASAFQAw%3D%3D";
  near(parseLatLng(url), 35.6215013, 139.8351224);
});

test("表示位置ではなく場所の座標を取る", () => {
  // ⚠️ 上のURLには `@35.621501,139.835122`（表示位置）と
  //    `!3d35.6215013!4d139.8351224`（場所そのもの）の両方が入っている。
  //    `@` を拾うと、地図をずらしただけで座標が変わってしまう
  const url = "https://www.google.com/maps/place/x/@35.100000,139.100000,16z"
    + "/data=!3m1!4b1!4m4!3m3!8m2!3d35.6215013!4d139.8351224";
  near(parseLatLng(url), 35.6215013, 139.8351224);
});

test("度分秒の数字を座標と取り違えない", () => {
  // ⚠️ URL の道中に 35°37'17.4"N のような数字が並ぶ。
  //    先頭から2つ拾うと 35 と 37 になってしまう
  const url = "https://www.google.com/maps/place/35%C2%B037'17.4%22N+139%C2%B050'06.4%22E"
    + "/@35.621501,139.835122,16z";
  const r = parseLatLng(url);
  assert.ok(r && Math.abs(r.lat - 35.621501) < 1e-5, JSON.stringify(r));
  assert.ok(r && Math.abs(r.lng - 139.835122) < 1e-5, JSON.stringify(r));
});
