...
県を選ぶと生成結果が点数順に並び、地図に上位30本が出る。行か線をクリックして選び、

| できること | 効果 |
|---|---|
| **点数の加算** | 押し上げ／押し下げ（マイナス可）。ビーナスラインは +15 で118位→1位になった |
| **表示名** | OSM の名前が実感と違うとき（湯河原箱根線 → 椿ライン）。元の名前は残る |
| **ひとこと説明** | アプリの一覧に1行で出る |
| **札** | 「絶景」などを最大2つまでアプリに出す |
| **一覧から外す** | 曲がってはいるが走って面白くない道を消す |

「保存」で `data/road-overrides/<romaji>.json` に書かれる。
...
画面の上部に警告が出て、まとめて削除できる。

### 重みの試算

スライダーで重みを動かすと、その県の上位40件がどう入れ替わるかが即座に出る
（`data/road-tuning/*.json` を使うので再生成は不要）。
