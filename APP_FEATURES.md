...
- **プラン一覧のマップ表示（Apple Maps）**：リスト⇄マップ切替。全プランのスポットを色分けピンで俯瞰し、上部チップでプランを絞り込み。訪問順を結ぶルート線、密集時のクラスタリング、ピンタップでスポット情報カード→「プランを開く」。（設計: `PLAN_MAP_DESIGN.md` / 実装: `saveRoute/PlanMapView.swift`）
- **おまかせ（ランダム）スポット提案**：現在地周辺からランダムにスポットを提案して行き先決めを支援。2モード：
  - **複数提案**：半径km以内から N 件提案。設定＝半径・件数・ジャンル/タグ・人気度重み。リスト＋地図でプレビュー→選択→「引き直し」可。
  - **どこ行こ**：行き先に迷ったとき用。目標距離（ユーザー設定）の**±20%あたりから完全ランダムで1件だけ**提示。**方角（東西南北）** で向きも絞れる。地図＋写真カードで「引き直し／マップで開く（経路）／プランに追加」。
  - 導線：プラン詳細の🎲で**既存プランへ一括追加**、プラン一覧/空状態の🎲で**おまかせ新規プラン作成**。
  - （設計: `PLAN_RANDOM_SUGGEST_DESIGN.md` / 実装: `saveRoute/RandomSpotSuggester.swift`, `saveRoute/RandomSpotSuggestView.swift`）
