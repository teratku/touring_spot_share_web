...
function toAppRoute(route) {
  const steps = route.steps.map((step, i) => ({
    ...
    // ⚠️ **曲がる地点の手前300mにある信号交差点までの距離（m・近い順）。** アプリは直前の案内で
    //    いまの位置から曲がる地点までの信号を数え、「2つ目の信号を右です」と言う（利用者の判断 2026-09-26）。
    //    古いアプリは読まない（今までどおり）
    signalsBefore: Array.isArray(step.signalsBefore) ? step.signalsBefore.filter(Number.isFinite) : [],
  ...
  return {
        ...
        // ⚠️ **経由地をずらした先。** 道の端・入口の輪（Uターン路）をほどくために、道の端や
        //    通る点を道に沿ってずらすことがある（`admin/lib/viaLoops.js`。上限は通る点300m・
        //    道の端1,000m）。アプリは地図のマーカーを実際に通る位置に描く（利用者の判断 2026-09-24:
        //    ツーリング3で国道286号の終点のマーカーが線から419m離れて見えた）。
        //    ⚠️ **番号でなく元の座標で渡す。** 重なった点をまとめる・区間ごとに引くと番号がずれる。
        //    向きを付けただけ（ずらしていない）ものは入れない
        movedVias: ((route.viaLoops && route.viaLoops.fixes) || [])
          .filter((f) => f.meters > 0 && Array.isArray(f.from) && Array.isArray(f.at))
          .map((f) => ({ from: f.from, to: f.at, meters: f.meters })),
        ...
        // ⚠️ **避けきれなかったスマートIC（ETC専用）は必ず返す。** ETC車載器が無いと出入りできない。
        //    車載器なし（`etc: false`）で頼まれたときだけ入る
        etcOnlyIcs: route.etcOnlyIcs,
  };
}
